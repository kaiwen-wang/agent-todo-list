...
async function submitPlanAnswer() {
  ...
  try {
    ...
    await loadPlan(); // Reload to show updated content
  } catch (e: unknown) {
...
// Also reload plan on WebSocket refresh (agent may have updated it)
watch(
  () => todo.value?.planPath,
  () => loadPlan(),
);
        ...
        <div class="detail-main">
          ...
          <div class="field-group">
            <label class="meta-label">
              <NIcon :size="12" style="vertical-align: -1px; margin-right: 4px"><FileText /></NIcon>
              Plan
            </label>
          ...
          <div class="field-group">
            ...
            <div v-if="todo.comments?.length" class="comments-list">
              <template v-for="c in threadedComments()" :key="c.id">
                <div class="comment-item" :style="{ marginLeft: `${c.depth * 24}px` }">
                ...
                <div
                  ...
                  :style="{ marginLeft: `${(c.depth + 1) * 24}px` }"
                >
                  <NInput
                    ref="replyInputRef"
