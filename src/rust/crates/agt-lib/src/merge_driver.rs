...
pub fn merge_files(_base_path: &Path, ours_path: &Path, theirs_path: &Path) -> Result<()> {
    ...
    let mut ours = AutoCommit::load(&ours_bytes).with_context(|| "failed to load ours")?;
    let mut theirs = AutoCommit::load(&theirs_bytes).with_context(|| "failed to load theirs")?;
